...
export function AppBar({ open, handleDrawerOpen }): React.ReactElement {
  ...
  const servicenow = `https://unicefdev.service-now.com/cc?id=sc_cat_item&sys_id=762ae3128747d91021cb670a0cbb35a7&${window.location.pathname.split('/')[1]}:${window.location.pathname.split('/')[2]}&${window.location.href}`
  if (meLoading) {
    return null;
  }
  return (
