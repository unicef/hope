...
import { EnrollmentCriteriaForm } from './EnrollmentCriteriaForm';
import {
  ContentWrapper,
...
export const EnrollmentCriteria = ({
  rules,
  helpers,
  targetPopulation,
  selectedProgram,
  isEdit,
}: EnrollmentCriteriaProps): React.ReactElement => {
  ...
  return (
    <div>
      <PaperContainer>
        <Title>
          <Typography variant='h6'>{t('Enrollment Criteria')}</Typography>
          {isEdit && (
            <>
              {!!rules.length && (
                <Button
        ...
        <ContentWrapper>
          {rules.length ? (
            ...
            <AddCriteria
              onClick={() => setOpen(true)}
              data-cy='button-enrollment-add-criteria'
            >
              <AddCircleOutline />
              <p>{t('Add Filter')}</p>
            </AddCriteria>
