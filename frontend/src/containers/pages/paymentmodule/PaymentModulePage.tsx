...
export function PaymentModulePage(): React.ReactElement {
  ...
  if (
    !hasPermissions(
      PERMISSIONS.PAYMENT_MODULE_PAYMENT_CYCLE_VIEW_LIST,
      permissions,
    )
  )
  ...
  return (
    <>
      <PageHeader title={t('Payment Module')}>
        {hasPermissions(
          PERMISSIONS.PAYMENT_MODULE_PAYMENT_CYCLE_CREATE,
          permissions,
        ) && (
          <Button
            variant='contained'
      ...
      <TableWrapper>
        <PaymentPlansTable
          ...
          canViewDetails={hasPermissions(
            PERMISSIONS.PAYMENT_MODULE_PAYMENT_CYCLE_VIEW_DETAILS,
            permissions,
          )}
        />
      </TableWrapper>
