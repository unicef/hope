...
context("Targeting", () => {
  ...
  it("Create a target population", () => {
    ...
    cy.uniqueSeed().then((seed) => {
      ...
      cy.get('[data-cy="button-individual-rule"]').as('addrule')
      cy.get('@addrule').click({ force: true })
      ...
      cy.get('[data-cy="button-target-population-lock"]').as('totalTarget')
      cy.get('@totalTarget').should('to.visible');
