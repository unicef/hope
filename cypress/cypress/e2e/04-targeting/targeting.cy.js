...
context("Targeting", () => {
  ...
  it("Create a target population", () => {
    ...
    cy.uniqueSeed().then((seed) => {
      ...
      cy.get('[data-cy="status-container"]').should('not.contain', "BUILDING");
      cy.get('[data-cy="label-Total Number of Households"]',{timeout:100000}).scrollIntoView().invoke('text').should('not.contain', ' 0 ');
      cy.get('[data-cy="label-Targeted Individuals"]').scrollIntoView().invoke('text').should('not.contain', ' 0 ');
