...
if [ "$AIRFLOW__CORE__EXECUTOR" != "SequentialExecutor" ]; then
  ...
  if [ -z "$AIRFLOW__CORE__SQL_ALCHEMY_CONN" ]; then
    ...
    : "${POSTGRES_EXTRAS:-""}"
    export POSTGRES_PORT=5432
    AIRFLOW__CORE__SQL_ALCHEMY_CONN="postgresql+psycopg2://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:5432/${POSTGRES_DB}${POSTGRES_EXTRAS}"
    ...
    if [ "$AIRFLOW__CORE__EXECUTOR" = "CeleryExecutor" ]; then
      AIRFLOW__CELERY__RESULT_BACKEND="db+postgresql://${POSTGRES_USER}:${POSTGRES_PASSWORD}@${POSTGRES_HOST}:5432/${POSTGRES_DB}${POSTGRES_EXTRAS}"
