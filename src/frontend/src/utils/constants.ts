...
export const getGrievanceCategoryDescriptions = (beneficiaryGroup) => ({
  DATA_CHANGE: `A grievance that is submitted to change in the ${beneficiaryGroup?.group_label_plural} or ${beneficiaryGroup?.member_label} status`,
  GRIEVANCE_COMPLAINT: `A grievance submitted to express dissatisfaction made about a ${beneficiaryGroup?.member_label}, UNICEF/NGO/Partner/Vendor, about a received service or about the process itself`,
  REFERRAL: `A grievance submitted to direct the reporting ${beneficiaryGroup?.member_label} to another service provider/actor to provide support/help that is beyond the scope of work of UNICEF`,
  SENSITIVE_GRIEVANCE: `A grievance that shall be treated with sensitivity or which ${beneficiaryGroup?.member_label} wishes to submit anonymously`,
  NEEDS_ADJUDICATION: `This is a ticket created for ${beneficiaryGroup?.member_label} records suspected of being potential duplicates. Such a ticket requires a review to decide the appropriate action, either to flag the ${beneficiaryGroup?.member_label_plural} as non-duplicates or to label the record as a duplicate.`,
  PAYMENT_VERIFICATION: `This refers to a system-generated ticket that is created due to a discrepancy between the amount delivered to the ${beneficiaryGroup?.member_label} (as outlined in the reconciliation report) and the actual amount received by the ${beneficiaryGroup?.member_label} (as reported during the payment verification process).`,
  SYSTEM_FLAGGING: `This ticket type is generated by the system for a ${beneficiaryGroup?.member_label} who potentially matches a ${beneficiaryGroup?.member_label} on the UN sanction list.`,
});

export const getGrievanceIssueTypeDescriptions = (beneficiaryGroup) => ({
  ADD_INDIVIDUAL: `A grievance submitted to specifically change in the ${beneficiaryGroup?.group_label_plural} to add a ${beneficiaryGroup?.member_label}`,
  EDIT_HOUSEHOLD: `A grievance submitted to change in the ${beneficiaryGroup?.group_label} data (Address, number of ${beneficiaryGroup?.member_label_plural}, etc.)`,
  EDIT_INDIVIDUAL: `A grievance submitted to change in the ${beneficiaryGroup?.group_label}’s ${beneficiaryGroup?.member_label_plural} data (Family name, full name, birth date, etc.)`,
  DELETE_INDIVIDUAL: `A grievance submitted to remove a ${beneficiaryGroup?.member_label} from within a ${beneficiaryGroup?.group_label}`,
  DELETE_HOUSEHOLD: `A grievance submitted to remove a ${beneficiaryGroup?.group_label}`,
  ...
  REGISTRATION_COMPLAINT: `A grievance submitted on issues/difficulties encountered during the registration of ${beneficiaryGroup?.member_label_plural}`,
  PARTNER_COMPLAINT:
  ...
  DATA_BREACH: `Grievance on unauthorized access or disclosure of ${beneficiaryGroup?.member_label} data`,
...
export const generateTableOrderOptionsGroup = (
  ...
  return [
    { name: `${beneficiaryGroup.group_label}: ascending`, value: 'unicef_id' },
    {
      name: `${beneficiaryGroup.group_label}: descending`,
      value: '-unicef_id',
    },
    { name: 'Status: ascending', value: 'status_label' },
    { name: 'Status: descending', value: '-status_label' },
    { name: `${beneficiaryGroup.group_label} Size: ascending`, value: 'size' },
    {
      name: `${beneficiaryGroup.group_label} Size: descending`,
      value: '-size',
    },
...
export const generateTableOrderOptionsMember = (
  ...
  return [
    { name: `${beneficiaryGroup.member_label}: ascending`, value: 'unicef_id' },
    {
      name: `${beneficiaryGroup.member_label}: descending`,
      value: '-unicef_id',
    },
    { name: 'Status: ascending', value: 'status_label' },
    { name: 'Status: descending', value: '-status_label' },
