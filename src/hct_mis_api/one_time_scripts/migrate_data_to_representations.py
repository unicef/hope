...
def migrate_data_to_representations_per_business_area(business_area: BusinessArea) -> None:
    ...
    for program in Program.objects.filter(
        ...
        target_populations_ids = PaymentPlan.objects.filter(
            program=program,
        ).values_list("id", flat=True)

        household_selections = Payment.objects.filter(
            Q(parent_id__in=target_populations_ids)
            & Q(is_original=True)
...
def copy_household_selections(household_selections: QuerySet["Payment"], program: Program) -> None:
    """
    ...
    for batch_start in range(0, len(household_selections_ids), BATCH_SIZE):
        ...
        batched_household_selections = Payment.objects.filter(id__in=batched_ids)
        logger.info(f"Copying household selections {batch_start} of {len(household_selections_ids)}")
        household_selections_to_create = []  # payments to create
        ...
        with transaction.atomic():
            Payment.objects.bulk_create(household_selections_to_create)
