...
class TestSyncRepresentations(BaseMigrateDataTestCase, BaseGrievanceTestCase, TestCase):
    ...
    def prepare_data(self) -> None:
        ...
        self.assertEqual(
            Household.original_and_repr_objects.filter(is_original=False, business_area=self.business_area).count(), 29
        )
        self.assertEqual(
            Individual.original_and_repr_objects.filter(is_original=False, business_area=self.business_area).count(), 44
        )
        self.assertEqual(
            IndividualRoleInHousehold.original_and_repr_objects.filter(
                is_original=False, household__business_area=self.business_area
            ).count(),
            20,
        )
        self.assertEqual(
            BankAccountInfo.original_and_repr_objects.filter(
                is_original=False, individual__business_area=self.business_area
            ).count(),
            16,
        )
        self.assertEqual(
    ...
    def test_create_new_objects(self) -> None:
        ...
        DocumentFactory(individual=new_head_of_household, program=None, is_original=True)
        IndividualIdentityFactory(individual=new_head_of_household, is_original=True)
        BankAccountInfoFactory(individual=new_head_of_household, is_original=True)
        ...
        self.assertEqual(new_household.copied_to(manager="original_and_repr_objects").all().count(), 2)

        for new_head_of_household_representation in new_head_of_household.copied_to.all():
            self.assertEqual(
                new_head_of_household_representation.documents(manager="original_and_repr_objects").all().count(), 1
            )
            self.assertEqual(
                new_head_of_household_representation.identities(manager="original_and_repr_objects").all().count(), 1
            )
            self.assertEqual(
                new_head_of_household_representation.bank_account_info(manager="original_and_repr_objects")
                .all()
                .count(),
                1,
            )
        ...
        DocumentFactory(individual=new_individual, program=None, is_original=True)
        IndividualIdentityFactory(individual=new_individual, is_original=True)
        BankAccountInfoFactory(individual=new_individual, is_original=True)
        ...
        for new_individual_representation in new_individual.copied_to(manager="original_and_repr_objects").all():
            self.assertEqual(
                new_individual_representation.documents(manager="original_and_repr_objects").all().count(), 1
            )
            self.assertEqual(
                new_individual_representation.identities(manager="original_and_repr_objects").all().count(), 1
            )
            self.assertEqual(
                new_individual_representation.bank_account_info(manager="original_and_repr_objects").all().count(), 1
            )
        ...
        for new_individual_representation in new_individual.copied_to.all():
            self.assertEqual(
                new_individual_representation.documents(manager="original_and_repr_objects").all().count(), 2
            )
            self.assertEqual(
                new_individual_representation.identities(manager="original_and_repr_objects").all().count(), 2
            )
            self.assertEqual(
                new_individual_representation.bank_account_info(manager="original_and_repr_objects").all().count(), 2
            )
