...
def migrate_cash_plan_to_payment_plan() -> None:
    ...
    batch_count = cash_plans.count() // 50
    batch_i = 0
    for batch in cash_plans.iterator(chunk_size=50):
        batch_i += 1
        logger.info(f"Processing batch {batch_i}/{batch_count}")
        for cp in batch:
            with transaction.atomic():
                ...
                for tp in target_populations:
                    ...
                    with transaction.atomic():
                        for record in cp.payment_items.filter(target_population=tp).prefetch_related(
                            "service_provider"
                        ):
                            financial_service_provider = fsp_vision_vendor_number_to_obj.get(
