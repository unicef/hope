...
class IndividualFilter(FilterSet):
    ...
    def filter_rdi_id(self, queryset: "QuerySet", model_field: Any, value: str) -> "QuerySet":
        ...
        extra_households = Household.extra_rdis.through.objects.filter(registrationdataimport=rdi_id).values_list("household_id", flat=True)
        return queryset.filter(Q(registration_data_import__pk=rdi_id)|Q(household__id__in=extra_households)).distinct()
