...
from hct_mis_api.apps.account.models import Partner, Role, User, USER_STATUS_CHOICES
...
class UserChoicesSerializer(serializers.Serializer):
    ...
    def get_role_choices(self, obj: User) -> List[Dict[str, Any]]:
        return [
            dict(name=role.name, value=role.id, subsystem=role.subsystem)
            for role in Role.objects.all().order_by("name")
        ]

    def get_status_choices(self, obj: User) -> List[Dict[str, Any]]:
