...
def migrate_user_roles_logic(apps: Any) -> None:
    """
...
def migrate_partner_roles_and_access_logic(apps: Any) -> None:
    """
    ...
    for partner_role in partner_roles:
        ...
        if programs:
            new_assignments.extend(
                RoleAssignment(user=None, partner=partner, role=role, business_area=business_area, program=program)
                for role in roles
    ...
    for access in area_access:
        new_area_limits.append(
            AdminAreaLimitedTo(partner=access.partner, program=access.program, areas=access.areas.all())
        )
    if new_area_limits:
        AdminAreaLimitedTo.objects.bulk_create(new_area_limits)


def migrate_unicef_partners_logic(apps: Any) -> None:
    """
    ...
    unicef_hq = Partner.objects.create(name=settings.UNICEF_HQ_PARTNER, parent=unicef_partner)
    unicef_hq.allowed_business_areas.set(BusinessArea.objects.all())

    for business_area in BusinessArea.objects.all():
        unicef_subpartner = Partner.objects.create(
            name=f"UNICEF Partner for {business_area.slug}", parent=unicef_partner
        )
