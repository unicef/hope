...
def migrate_partner_roles_and_access(apps, schema_editor):
    ...
    for batch_start in range(0, partner_roles_count, BATCH_SIZE):
        ...
        for partner_role in batched_partner_roles:
            ...
            roles = partner_role.roles.exclude(name="Role with all permissions")  # in case some existing Partner on any anv has "Role with all permissions" - which is not allowed for Partners anymore
