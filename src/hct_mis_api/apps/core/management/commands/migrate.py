...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        target_apps = [
            ...
            if settings.DATABASE_APPS_MAPPING.get(app.label, "default") == db
        ]
        ...
        if options["app_label"]:
            ...
            if run_syncdb:
                if app_label in executor.loader.migrated_apps:
                    raise CommandError(f"Can't use run_syncdb with app '{app_label}' as it has migrations.")
        ...
        if options["app_label"] and options["migration_name"]:
            ...
            targets = [key for key in executor.loader.graph.leaf_nodes() if key[0] in target_apps]
        ...
        if self.verbosity >= 1:
            ...
            if target_app_labels_only:
                ...
                self.stdout.write(self.style.MIGRATE_LABEL("  Unapply all migrations: ") + f"{targets[0][0]}")
            else:
                self.stdout.write(
                    self.style.MIGRATE_LABEL("  Target specific migration: ") + f"{targets[0][1]}, from {targets[0][0]}"
                )
    ...
    def migration_progress_callback(self, action: str, migration: str | None = None, fake: bool | None = False) -> None:
        if self.verbosity >= 1:
