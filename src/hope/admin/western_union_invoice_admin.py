...
class WesternUnionInvoiceAdmin(AutocompleteForeignKeyMixin, admin.ModelAdmin):
    ...
    list_display = ["name", "date", "net_amount", "charges", "status", "matched_data", "payment_plans_list"]

    list_filter = ["status", "date"]
    search_fields = ["name", "matched_data__name", "reports__payment_plan__unicef_id", "reports__payment_plan__name"]
