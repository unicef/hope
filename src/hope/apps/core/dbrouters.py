...
class DbRouter:
    ...
    def db_for_read(self, model: type[Model] | None, **hints: object) -> str | None:
        return DbRouter.select_db(model)

    def db_for_write(self, model: type[Model] | None, **hints: object) -> str | None:
        return DbRouter.select_db(model)

    def allow_migrate(self, db: str, app_label: str, model_name: str | None = None, **hints: object) -> bool:
        if db == "ro":
            return False
