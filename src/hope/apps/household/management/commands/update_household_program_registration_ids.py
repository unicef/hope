...
class Command(BaseCommand):
    ...
    def handle(self, *args: str, **options: str) -> None:
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Business area with slug '{business_area_slug}' not found"))
            return
        ...
        if not active_programs.exists():
            self.stdout.write(self.style.WARNING(f"No active programs found in business area '{business_area_slug}'"))
            return

        self.stdout.write(
            self.style.SUCCESS(f"Found {active_programs.count()} active program(s) in '{business_area_slug}'")
        )
        ...
        if not households.exists():
            self.stdout.write(self.style.WARNING(f"No households found in active programs in '{business_area_slug}'"))
            return
        ...
        for household in households:
            ...
            if dry_run:
                self.stdout.write(f"[DRY RUN] HH {household.unicef_id}: '{old_id}' -> '{new_id}'")
                updated_count += 1
            else:
                try:
                    household.program_registration_id = new_id
                    household.save(update_fields=["program_registration_id"])
                    self.stdout.write(self.style.SUCCESS(f"✓ HH {household.unicef_id}: '{old_id}' -> '{new_id}'"))
                    updated_count += 1
                except Exception as e:  # noqa: BLE001
                    self.stdout.write(self.style.ERROR(f"✗ HH {household.unicef_id}: Failed - {str(e)}"))
                    failed_count += 1
