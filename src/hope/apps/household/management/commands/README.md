...
| `es_reindex` | The recurring blue-green reindex (requires bootstrap done): per program, create a dark `_vN+1` pair from the code mapping (one lockstep version for both indexes), full-populate, one delta `--target-suffix` catch-up pass, count-verify vs Postgres (one retry delta, abort before swap on mismatch), ONE atomic 4-action alias swap (`must_exist` + postcondition), two post-swap delta passes. The old version stays unaliased as an instant rollback target. Crash recovery = RESUME: the working version is deterministic (alias target + 1), a leftover with a matching mapping stamp (`_meta.hope_mapping_hash`) skips the full populate and is topped up by a delta from its `creation_date`; a stale-mapping leftover is rebuilt automatically under the same number; a half-populated one fails the verify gate with a `--sweep-wrecks` hint. |
