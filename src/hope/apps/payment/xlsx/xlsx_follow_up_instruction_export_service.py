...
class XlsxFollowUpInstructionExportService(XlsxExportBaseService):
    ...
    def __init__(self, instruction: FollowUpInstruction):
        ...
        self.helper = XlsxPaymentPlanExportPerFspService(self.payment_plan, None)
        ...
        fsp_xlsx_template = self.helper.get_template(fsp, delivery_mechanism)
        self.helper.prepare_headers(fsp_xlsx_template)  # type: ignore[arg-type]
        self.headers = self._prepare_headers()

    def _get_representative_payment_plan(self) -> PaymentPlan:
        payment_plan = self.instruction.payment_plans.select_related(
            "delivery_mechanism",
    ...
    def _prepare_headers(self) -> list[str]:
        headers = list(self.helper.header_list)
    ...
    def _build_payment_row(self, payment: Payment) -> dict[str, Any]:
        payment_row = dict(zip(self.helper.header_list, self.helper.get_payment_row(payment), strict=True))
    ...
    def generate_workbook(self):  # type: ignore[override]
    ...
    def save_xlsx_file(self, user: User) -> None:
        filename = f"follow_up_instruction_payment_list_{self.instruction.unicef_id or self.instruction.id}.xlsx"
