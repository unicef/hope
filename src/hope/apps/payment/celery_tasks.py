...
def import_payment_plan_payment_list_from_xlsx_async_task_action(job: AsyncRetryJob) -> None:
    ...
    with celery_lock("import_payment_plan_payment_list_from_xlsx", job.config["payment_plan_id"]):
        payment_plan = PaymentPlan.objects.get(id=job.config["payment_plan_id"])
        set_sentry_business_area_tag(payment_plan.business_area.name)
        old_payment_plan = cast("PaymentPlan", copy_model_object(payment_plan))
...
def payment_plan_set_entitlement_flat_amount_async_task_action(job: AsyncRetryJob) -> None:
    ...
    with celery_lock("payment_plan_set_entitlement_flat_amount", job.config["payment_plan_id"]):
        payment_plan = PaymentPlan.objects.get(id=job.config["payment_plan_id"])
        set_sentry_business_area_tag(payment_plan.business_area.name)
...
def payment_plan_apply_custom_exchange_rate_async_task_action(job: AsyncRetryJob) -> None:
    ...
    with celery_lock("payment_plan_apply_custom_exchange_rate", job.config["payment_plan_id"]):
...
def import_payment_plan_delivery_from_xlsx_async_task_action(job: AsyncRetryJob) -> bool:
    ...
    with celery_lock("import_payment_plan_delivery_from_xlsx", job.config["payment_plan_id"]):
...
def import_payment_plan_fsp_extra_fields_from_xlsx_async_task_action(job: AsyncRetryJob) -> bool:
    ...
    with celery_lock("import_payment_plan_fsp_extra_fields_from_xlsx", job.config["payment_plan_id"]):
...
def import_follow_up_instruction_reconciliation_from_xlsx_async_task_action(job: AsyncRetryJob) -> bool:
    ...
    with celery_lock("import_follow_up_instruction_reconciliation_from_xlsx", job.config["follow_up_instruction_id"]):
...
def import_payment_plan_group_delivery_from_xlsx_async_task_action(job: AsyncRetryJob) -> None:
    ...
    with celery_lock("import_payment_plan_group_delivery_from_xlsx", job.config["payment_plan_group_id"]):
...
def payment_plan_apply_engine_rule_async_task_action(job: AsyncRetryJob) -> None:  # noqa: PLR0915
    ...
    with celery_lock("payment_plan_apply_engine_rule", job.config["payment_plan_id"]):
        payment_plan = get_object_or_404(PaymentPlan, id=job.config["payment_plan_id"])
        set_sentry_business_area_tag(payment_plan.business_area.name)
        old_payment_plan = cast("PaymentPlan", copy_model_object(payment_plan))
...
def payment_plan_exclude_beneficiaries_async_task_action(job: AsyncRetryJob) -> None:  # noqa: PLR0915
    ...
    with celery_lock("payment_plan_exclude_beneficiaries", job.config["payment_plan_id"]):
        ...
        try:
            ...
            excluding_hh_or_ind_ids = [
                unicef_id for unicef_id in excluding_hh_or_ind_ids if unicef_id in payment_item_ids
            ]
...
def send_to_payment_gateway_async_task_action(job: AsyncJob) -> None:
    ...
    with celery_lock("send_to_payment_gateway", job.config["payment_plan_id"]):
        payment_plan = PaymentPlan.objects.get(id=job.config["payment_plan_id"])
        user = User.objects.get(pk=job.config["user_id"])
...
def payment_plan_apply_steficon_hh_selection_async_task_action(job: AsyncRetryJob) -> None:  # noqa: PLR0915
    from hope.models import Payment, PaymentPlan, Rule, RuleCommit

    with celery_lock("payment_plan_apply_steficon_hh_selection", job.config["payment_plan_id"]):
        payment_plan = get_object_or_404(PaymentPlan, id=job.config["payment_plan_id"])
        set_sentry_business_area_tag(payment_plan.business_area.name)
        old_payment_plan = cast("PaymentPlan", copy_model_object(payment_plan))
        engine_rule = get_object_or_404(Rule, id=job.config["engine_rule_id"])
