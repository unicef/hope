...
from hope.models import AsyncRetryJob, BusinessArea, Document, Program, RegistrationDataImport
...
def process_country_workspace_rdis_task_action(job: AsyncRetryJob) -> bool:
    rdis = RegistrationDataImport.objects.filter(
        ...
        business_area__ingest_source=BusinessArea.IngestSource.COUNTRY_WORKSPACE_ONLY,
    )
    for rdi in rdis:
