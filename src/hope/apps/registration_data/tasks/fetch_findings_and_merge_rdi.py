...
class FetchFindingsAndMergeRdi:
    ...
    def execute(self, registration_data_import_id: str) -> bool:
        ...
        try:
            with transaction.atomic():
                rdi = self._lock_rdi(registration_data_import_id)
                if rdi is None:
                    return False
                self._transition_to_merging(rdi)
                self._store_deduplication_results(rdi, dedupe_service, findings)
    ...
    def _load_and_guard_rdi(self, registration_data_import_id: str) -> RegistrationDataImport | None:
        rdi = (
