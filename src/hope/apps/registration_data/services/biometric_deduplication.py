...
class BiometricDeduplicationService:
    ...
    def get_deduplication_set_results(self, program: Program, individual_ids: list[str]) -> list[dict]:
        results: list[dict] = []
        for i in range(0, len(individual_ids), self.GET_DUPLICATES_BATCH_SIZE):
            ...
            results.extend(self.api.get_duplicates(program.unicef_id, batch))
        return results

    def get_deduplication_set(self, program: Program) -> DeduplicationSetData:
        response_data = self.api.get_deduplication_set(program.unicef_id)
        return DeduplicationSetData(state=response_data["state"], error=response_data.get("error", ""))

    def upload_individuals(self, program: Program, rdi: RegistrationDataImport) -> None:
        individuals = (
        ...
        if rdi.deduplication_engine_status in [
            ...
            try:
                ...
                logging.exception("Failed to upload images for RDI %s", safe_log(rdi))
    ...
    def process_deduplication_set(self, program: Program, rdis: QuerySet[RegistrationDataImport]) -> None:
        try:
            response_data, status = self.api.process_deduplication(program.unicef_id)
            if status in [409, 200]:  # 409 - deduplication already in progress
                ...
                logging.error(
                    ...
                    safe_log(program.unicef_id),
                    status,
                    safe_log(response_data),
                )
            ...
            logging.exception("Failed to process deduplication set %s", safe_log(program.unicef_id))
            rdis.update(deduplication_engine_status=RegistrationDataImport.DEDUP_ENGINE_ERROR)
    ...
    def upload_and_process_deduplication_set(self, program: Program) -> None:
        ...
        for rdi in pending_rdis:
            self.upload_individuals(program, rdi)
        ...
        self.process_deduplication_set(program, uploaded_rdis)

    def delete_deduplication_set(self, program: Program) -> None:
        self.api.delete_deduplication_set(program.unicef_id)
    ...
    def mark_rdis_as_deduplicated(rdis: QuerySet[RegistrationDataImport]) -> None:
        rdis.update(deduplication_engine_status=RegistrationDataImport.DEDUP_ENGINE_FINISHED)

    @staticmethod
    def mark_rdis_as_error(rdis: QuerySet[RegistrationDataImport]) -> None:
        rdis.update(deduplication_engine_status=RegistrationDataImport.DEDUP_ENGINE_ERROR)
    ...
    def fetch_biometric_deduplication_results_and_process(
        ...
        if deduplication_set_data.state == self.DEDUP_STATE_READY:
            try:
                ...
                logger.exception("Dedupe Engine processing results error for program %s", safe_log(program))
                self.mark_rdis_as_error(rdis)
                invalidate_rdi_cache(program.business_area.slug, program.code)

        elif deduplication_set_data.state == self.DEDUP_STATE_FAILED:
            logger.error(
                "Dedupe Engine error for program %s: %s", safe_log(program), safe_log(deduplication_set_data.error)
            )
            self.mark_rdis_as_error(rdis)
            invalidate_rdi_cache(program.business_area.slug, program.code)
