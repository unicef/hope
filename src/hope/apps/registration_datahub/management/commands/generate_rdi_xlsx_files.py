...
def get_individual_header_mapping() -> dict:
    return {
...
def generate_rdi_xlsx_files(*args: Any, **options: Any) -> None:
    ...
    for index, (_, (header, _)) in enumerate(get_household_header_mapping().items()):
    ...
    for count in range(amount):
        for index, (key, (_, value)) in enumerate(get_individual_header_mapping().items()):
            if value is None:
                continue
            if key in ["AY", "A"]:
