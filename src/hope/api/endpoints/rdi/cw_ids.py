...
def collect_cw_ids(request_data: Iterable[object]) -> list[str]:
    """Country workspace ids of a flat individual payload."""
    return [
        row["country_workspace_id"]
        for row in request_data
        if isinstance(row, dict) and isinstance(row.get("country_workspace_id"), str) and row["country_workspace_id"]
    ]
...
def collect_member_cw_ids(request_data: Iterable[object]) -> list[str]:
    ...
    for household in request_data:
        ...
        for member in household.get("members") or []:
            ...
            if isinstance(cw_id, str) and cw_id:
                cw_ids.append(cw_id)
    return cw_ids
