...
def individual(
    ...
    ind.sex = MALE
    ind.given_name = "Test Name"
    ind.flex_fields = {"muac": 25}
    ind.save(update_fields=["unicef_id", "sex", "phone_no", "given_name", "flex_fields"])
    return ind
...
@pytest.fixture
def household_with_eur(program: Program, currencies: dict[str, Currency]) -> Household:
...
def test_update_individual(
    ...
    individual.household.currency = currencies["EUR"]
    ...
    household.currency = currencies["USD"]
    household.save()
...
def test_validate_currency_valid_code(business_area: object, program: Program, currencies: dict[str, Currency]) -> None:
    assert validate_currency("USD", "currency", Household, business_area, program) is None


def test_validate_currency_unknown_code(
    business_area: object, program: Program, currencies: dict[str, Currency]
) -> None:
    error = validate_currency("ZZZ", "currency", Household, business_area, program)
    assert error == "Invalid currency code ZZZ"


def test_validate_currency_empty_string(
    business_area: object, program: Program, currencies: dict[str, Currency]
) -> None:
...
def test_household_update_currency_eur_to_usd(
    ...
    assert household_with_eur.currency == currencies["USD"]
...
def test_household_update_currency_ignore_empty_true(
    ...
    household_with_eur.save()
    household_with_eur.refresh_from_db()
    assert household_with_eur.currency == currencies["EUR"]
...
def test_get_generator_handler_renders_currency_code(currencies: dict[str, Currency]) -> None:
    usd = currencies["USD"]
