...
@pytest.fixture
def pdu_online_edit(afghanistan: BusinessArea, program: Program, user: User) -> PDUOnlineEdit:
...
@pytest.fixture
def initial_request_pdu_fields_data() -> dict:
    return {
        "vaccination_records": {
            "round_number": 1,
            "value": None,
...
def get_save_data_url(afghanistan: BusinessArea, program: Program, pdu_edit_id: str) -> str:
    return reverse(
...
def test_save_data_permissions(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "vaccination_records", 10.5)
...
def test_save_data_success_decimal_field(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "vaccination_records", 15.5)
    ...
    pdu_online_edit.refresh_from_db()
    individual_data = pdu_online_edit.edit_data[0]
...
def test_save_data_success_string_field(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "health_check", "Good condition")
...
def test_save_data_success_boolean_field(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "health_status", True)
...
def test_save_data_success_date_field(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "visit_date", "2024-01-15")
...
def test_save_data_no_value_change(
    ...
    data = {
        "individual_uuid": "2b9a1db2-23bd-46a8-8a2c-1d51891d6f6b",
        "pdu_fields": initial_request_pdu_fields_data,
    }

    edit_data_before = pdu_online_edit.edit_data.copy()
...
def test_save_data_not_authorized_user(
    ...
    pdu_online_edit.authorized_users.remove(user)
...
def test_save_data_invalid_status(
    ...
    pdu_online_edit.status = PDUOnlineEdit.Status.APPROVED
    pdu_online_edit.save()
...
def test_serializer_validation_field_data_not_dict(
    ...
    data = {
        "individual_uuid": "2b9a1db2-23bd-46a8-8a2c-1d51891d6f6b",
        "pdu_fields": {"vaccination_records": "invalid"},
    }
    response = authenticated_client.post(url, data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert (
        'Expected a dictionary of items but got type "str".' in response.json()["pdu_fields"]["vaccination_records"][0]
    )
...
def test_serializer_validation_missing_required_keys(
    ...
    data = {
        ...
        "pdu_fields": {
            "vaccination_records": {
                "value": 10.5,  # Missing required keys: subtype, is_editable, round_number
            },
        },
    }
    response = authenticated_client.post(url, data=data)
...
def test_serializer_validation_non_editable_field_modification(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "completed_vaccination", False)
    ...
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert (
        "Field 'completed_vaccination' is not editable and cannot be modified" in response.json()["non_field_errors"][0]
    )
...
def test_serializer_validation_invalid_boolean_type(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "health_status", "invalid_bool")
...
def test_serializer_validation_invalid_decimal_type(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "vaccination_records", "not_a_number")
...
def test_serializer_validation_invalid_string_type(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "health_check", 123)
...
def test_serializer_validation_invalid_date_type(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "visit_date", 20240115)
...
def test_serializer_validation_invalid_date_format(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "visit_date", "15/01/2024")
    ...
    response = authenticated_client.post(url, data=data)
    assert response.status_code == status.HTTP_400_BAD_REQUEST
    assert "Field 'visit_date' has invalid date format. Expected YYYY-MM-DD" in response.json()["non_field_errors"][0]
...
def test_save_data_multiple_field_types_in_one_request(
    ...
    url = get_save_data_url(afghanistan, program, pdu_online_edit.id)
    updated_fields = create_request_data(initial_request_pdu_fields_data, "vaccination_records", 25.5)
    updated_fields = create_request_data(updated_fields, "health_check", "Excellent")
    updated_fields = create_request_data(updated_fields, "health_status", True)
    updated_fields = create_request_data(updated_fields, "visit_date", "2024-02-01")
