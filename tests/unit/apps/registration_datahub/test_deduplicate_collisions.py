...
from hct_mis_api.apps.program.collision_detectors import IdentificationKeyCollisionDetector
...
def program(poland: Country) -> Program:
    ...
    program = ProgramFactory(name="Test Program for Deduplication", status=Program.ACTIVE, business_area=business_area)
...
@pytest.fixture()
def merged_household(program: Program, registration_data_import_merged: RegistrationDataImport):
    ...
    return (household, inds[0])


@pytest.fixture()
def pending_household(program: Program, importing_registration_data_import: RegistrationDataImport):
    household, inds = create_household_and_individuals(
        household_data={
            "program": program,
            "business_area": program.business_area,
...
def test_collided_individuals_ids_to_exclude(
    merged_household, pending_household, importing_registration_data_import, program
):
...
def test_deduplicate_pending_individuals_skips_collided(
    merged_household, pending_household, importing_registration_data_import, program, mocker
):
    (merged_household1, merged_individual1) = merged_household
    (pending_household1, pending_individual1) = pending_household
