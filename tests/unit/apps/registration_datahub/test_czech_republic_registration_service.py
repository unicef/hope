...
class TestCzechRepublicRegistrationService(TestCase):
    ...
    def test_import_data_to_datahub(self) -> None:
        ...
        assert bank_account_info.bank_account_number == "CZ6003000000000306979952"
        assert bank_account_info.bank_branch_name == "Branch Name CZ"
        assert bank_account_info.account_holder_name == "Test Holder Name CZ"
        assert bank_account_info.rdi_merge_status == "PENDING"
        ...
        assert medical_certificate.expiry_date == datetime.datetime(2023, 5, 17, tzinfo=pytz.UTC)
        assert medical_certificate.issuance_date == datetime.datetime(2023, 5, 1, tzinfo=pytz.UTC)
