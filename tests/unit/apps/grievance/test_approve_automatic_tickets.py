...
class TestGrievanceApproveAutomaticMutation(APITestCase):
    ...
    def test_approve_needs_adjudication_allows_multiple_selected_individuals_with_permission(self) -> None:
        ...
        assert grievance_ticket_id == response_data["id"]
        assert self.id_to_base64(self.individuals[0].id, "IndividualNode") in selected_individuals_ids
        assert self.id_to_base64(self.individuals[1].id, "IndividualNode") in selected_individuals_ids
