...
class TestSignalChangeAllowedBusinessAreas(TestCase):
    ...
    def test_signal_change_allowed_business_areas(self) -> None:
        ...
        self.assertIsNotNone(
            self.partner.role_assignments.filter(program=self.program_afg).first()
        )
        self.assertIsNotNone(
            self.partner.role_assignments.filter(program=None, business_area=self.business_area_afg).first()
        )
        self.assertIsNotNone(
            self.partner.role_assignments.filter(program=self.program_ukr_2).first()
        )

        self.assertEqual(
            AdminAreaLimitedTo.objects.filter(partner=self.partner).count(),
            0
        )
