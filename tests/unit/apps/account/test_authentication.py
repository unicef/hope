...
class TestSocialDetails:
    ...
    def test_social_details_returns_email_from_original_details_when_present(
        self, mock_social_auth_details: MagicMock, mock_backend: MagicMock
    ) -> None:
    ...
    @patch("hope.apps.account.authentication.social_auth.social_details")
    def test_social_details_keeps_email_none_when_user_data_returns_none(
        self, mock_social_auth_details: MagicMock, mock_backend: MagicMock
    ) -> None:
        mock_social_auth_details.return_value = {"details": {}}
        mock_backend.user_data.return_value = None
        response = {}
...
class TestUserDetails:
    ...
    def test_user_details_updates_username_to_email_when_user_exists(
        self, mock_social_core_user_details: MagicMock, user: User, mock_strategy: MagicMock, mock_backend: MagicMock
    ) -> None:
    ...
    def test_user_details_skips_update_when_user_is_none(
        self, mock_social_core_user_details: MagicMock, mock_strategy: MagicMock, mock_backend: MagicMock
    ) -> None:
        details = {"email": "test@example.com"}
        mock_social_core_user_details.return_value = None
...
class TestRequireEmail:
    def test_require_email_returns_none_when_user_has_email(self, user: User, mock_strategy: MagicMock) -> None:
        result = require_email(mock_strategy, {}, user=user, is_new=False)

        assert result is None

    def test_require_email_returns_none_when_user_none_and_not_new(self, mock_strategy: MagicMock) -> None:
        result = require_email(mock_strategy, {}, user=None, is_new=False)

        assert result is None

    def test_require_email_returns_none_when_new_user_with_email_in_details(self, mock_strategy: MagicMock) -> None:
        details = {"email": "new@example.com"}
    ...
    def test_require_email_raises_invalid_email_when_new_user_without_email(self, mock_strategy: MagicMock) -> None:
...
class TestCreateUser:
    def test_create_user_returns_is_new_false_when_user_exists(
        self, user: User, mock_strategy: MagicMock, mock_backend: MagicMock
    ) -> None:
        result = create_user(mock_strategy, {}, mock_backend, user)

        assert result == {"is_new": False}

    @patch("hope.apps.account.authentication.MicrosoftGraphAPI")
    def test_create_user_creates_user_with_email_and_names(
        self, mock_ms_graph_class: MagicMock, mock_strategy: MagicMock, mock_backend: MagicMock
    ) -> None:
        mock_ms_graph = MagicMock()
        mock_ms_graph.get_user_data.return_value = {}
        mock_ms_graph_class.return_value = mock_ms_graph
        details = {
    ...
    def test_create_user_creates_role_assignment_when_business_area_code_exists(
        ...
        role_assignment = RoleAssignment.objects.get(user=result["user"])
    ...
    @patch("hope.apps.account.authentication.MicrosoftGraphAPI")
    def test_create_user_returns_is_new_true_with_user_object(
        self, mock_ms_graph_class: MagicMock, mock_strategy: MagicMock, mock_backend: MagicMock
    ) -> None:
        mock_ms_graph = MagicMock()
        mock_ms_graph.get_user_data.return_value = {}
