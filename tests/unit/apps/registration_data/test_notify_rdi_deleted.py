...
def test_notify_enqueue_builds_config() -> None:
    ...
    assert queue_task.call_args.kwargs["config"] == {"callback_url": CALLBACK_URL}
    assert queue_task.call_args.kwargs["action"] == ACTION_PATH


def test_notify_action_calls_cw_api() -> None:
    job = AsyncRetryJob(config={"callback_url": CALLBACK_URL})
    ...
    cw_api.return_value.notify_rdi_deleted.assert_called_once_with()
