...
def countries() -> dict:
    ...
    ukraine = CountryFactory(name="Ukraine", short_name="Ukraine", iso_code2="UA", iso_code3="UKR", iso_num="0804")
...
@pytest.fixture
def validator(program: object) -> KoboProjectImportDataInstanceValidator:
    return KoboProjectImportDataInstanceValidator(program)


def test_image_validator(validator: KoboProjectImportDataInstanceValidator) -> None:
    valid_attachments = [
        {
            "mimetype": "image/png",
            "download_small_url": "https://kc.humanitarianresponse.info/"
...
def test_geopoint_validator_valid(
    validator: KoboProjectImportDataInstanceValidator,
    value: str,
) -> None:
    assert validator.geopoint_validator(value, "hh_geopoint_h_c") is None
...
def test_geopoint_validator_invalid(
    validator: KoboProjectImportDataInstanceValidator,
    value: object,
) -> None:
    assert (
        ...
        == f"Invalid geopoint {value} for field hh_geopoint_h_c"
    )
...
@pytest.mark.parametrize(
    ("value", "expected"),
    [
        ("2020-05-28T17:13:31.590+02:00", None),
        ("2020-05-28", None),
        (
        ...
        (
            None,
            "Invalid datetime/date None for field birth_date_i_c, accepted formats: datetime ISO 8601, date YYYY-MM-DD",
        ),
    ],
)
def test_date_validator(
    ...
    result = validator.date_validator(value, "birth_date_i_c")
    assert result == expected
...
@pytest.mark.parametrize(
    ("args", "expected"),
    [
        (("size_h_c", 4, VALID_JSON[0]["_attachments"]), None),
        (
            ("size_h_c", "four", VALID_JSON[0]["_attachments"]),
            {
                "header": "size_h_c",
                "message": "Invalid value four of type str for field size_h_c of type int",
            },
        ),
        (("address_h_c", "Street 123", VALID_JSON[0]["_attachments"]), None),
        (("address_h_c", 123, VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", True, VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", "True", VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", False, VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", "False", VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", 1, VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", "1", VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", 0, VALID_JSON[0]["_attachments"]), None),
        (("returnee_h_c", "0", VALID_JSON[0]["_attachments"]), None),
        (
            ("returnee_h_c", "123", VALID_JSON[0]["_attachments"]),
            {
                "header": "returnee_h_c",
        ...
        (("gender_i_c", "MALE", VALID_JSON[0]["_attachments"]), None),
        (
            ("gender_i_c", "YES", VALID_JSON[0]["_attachments"]),
            {
                "header": "gender_i_c",
        ...
        (
            ("birth_date_i_c", "2020-05-28T17:13:31.590+02:00", VALID_JSON[0]["_attachments"]),
            None,
        ),
        (
            ("birth_date_i_c", "2020/05/28", VALID_JSON[0]["_attachments"]),
            {
        ...
        (("hh_geopoint_h_c", "12.123 13.123", VALID_JSON[0]["_attachments"]), None),
        (
            ("hh_geopoint_h_c", "GeoPoint 12.123, 32.123", VALID_JSON[0]["_attachments"]),
            {
...
def test_get_field_type_error(
    ...
    result = validator._get_field_type_error(*args)
    assert result == expected
...
def test_validate_everything(
    ...
    result = validator.validate_everything(VALID_JSON, business_area, True)
    assert result == []

    business_area.custom_fields = {"hope": {"ignore_amended_kobo_submissions": True}}
    business_area.save()
    result = validator.validate_everything(VALID_JSON, business_area, False)
    assert result == []

    result = validator.validate_everything(INVALID_JSON, business_area)
...
def test_check_error_list_validate_everything(
    program: object,
    counts: tuple[int, int, int],
    expected: list[dict],
) -> None:
    validator = KoboProjectImportDataInstanceValidator(program)
    validator._process_error_msg(*counts)
