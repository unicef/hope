...
def queue_and_run_retry_task(task: object, *args: object, **kwargs: object) -> object:
    ...
    # RdiMergeTask (and downstream tasks) may register transaction.on_commit
    # callbacks, which never fire under django_db's rollback wrapping — capture
    # and execute them so those paths are actually exercised.
...
def test_fetch_and_merge_fetch_failure_before_txn_is_merge_error(
    ...
    # N4: get_rdi_findings raises BEFORE the txn opens → RDI still ends in MERGE_ERROR
    # (every failure in this task does), but nothing is written, the merge never runs,
    # and — because no ES doc was ever indexed — there is no ES cleanup.
...
def test_action_does_not_reenqueue_dispatcher_on_failure(
    ...
    exc = RuntimeError("merge exploded")
    mock_task_cls.return_value.execute.side_effect = exc
    job = AsyncRetryJob(config={"registration_data_import_id": str(cw_rdi.id), "program_id": str(cw_rdi.program_id)})

    with pytest.raises(RuntimeError, match="merge exploded"):
        fetch_findings_and_merge_rdi_action(job)

    mock_handle.assert_called_once_with(str(cw_rdi.id), exc, new_status=RegistrationDataImport.MERGE_ERROR)
    mock_dispatcher.assert_not_called()
