...
@patch("hope.apps.registration_data.celery_tasks.deduplication_engine_process")
def test_run_deduplication(
    ...
    mock_deduplication_engine_process.assert_called_once_with(program)
...
@patch("hope.apps.registration_data.celery_tasks.fetch_biometric_deduplication_results_and_process")
def test_webhook_deduplication(mock_fetch_dedup_results: Mock, api_client: APIClient, program: Program) -> None:
    ...
    mock_fetch_dedup_results.assert_called_once_with(program)
...
@patch("hope.apps.registration_data.celery_tasks.merge_registration_data_import_task")
def test_merge_rdi(mock_merge_task: Mock, api_client: APIClient, program: Program, business_area: BusinessArea) -> None:
    ...
    mock_merge_task.assert_called_once_with(registration_data_import_id=rdi)
...
@patch("hope.apps.registration_data.celery_tasks.rdi_deduplication_task")
...
@patch("hope.apps.registration_data.celery_tasks.registration_program_population_import_task")
