...
from hope.apps.geo.celery_tasks import import_areas_from_csv_task, import_areas_from_csv_task_action
...
def test_import_areas_from_csv(country: Country, delay_mptt_updates: bool) -> None:
    ...
    import_areas_from_csv_task_action(create_async_job(csv_data_create, delay_mptt_updates))
    ...
    import_areas_from_csv_task_action(create_async_job(csv_data_update, delay_mptt_updates))
...
def test_import_areas_from_csv_task_action_clears_errors_on_success(country: Country) -> None:
    ...
    import_areas_from_csv_task_action(job)
...
def test_import_areas_from_empty_csv() -> None:
    ...
    with pytest.raises(IndexError):
        import_areas_from_csv_task_action(create_async_job(csv_data_empty))

    csv_data_header_only = "Country,State,County,country_pcode,state_pcode,county_pcode\n"
    with pytest.raises(IndexError):
        import_areas_from_csv_task_action(create_async_job(csv_data_header_only))
...
def test_import_areas_from_csv_no_country() -> None:
    ...
    with pytest.raises(Country.DoesNotExist):
        import_areas_from_csv_task_action(create_async_job(csv_data))
...
def test_import_areas_from_csv_task_schedules_async_job(mock_queue: Mock) -> None:
    ...
    import_areas_from_csv_task(csv_data, delay_mptt_updates=True)

    job = AsyncJob.objects.get()

    assert job.owner is None
    assert job.type == "JOB_TASK"
    assert job.action == "hope.apps.geo.celery_tasks.import_areas_from_csv_task_action"
    assert job.config == {"csv_data": csv_data, "delay_mptt_updates": True}
    assert job.group_key == "import_areas_from_csv_task"
