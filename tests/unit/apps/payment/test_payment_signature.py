...
class TestPaymentSignature(BaseTestCase):
    ...
    def calculate_hash_manually(self, payment: Payment) -> str:
        ...
        sha256.update(settings.SECRET_KEY.encode("utf-8"))
        sha256.update(str(payment.parent_id).encode("utf-8"))
        sha256.update(str(payment.conflicted).encode("utf-8"))
        sha256.update(str(payment.excluded).encode("utf-8"))
        sha256.update(str(payment.entitlement_date).encode("utf-8"))
        sha256.update(str(payment.financial_service_provider_id).encode("utf-8"))
        sha256.update(str(payment.collector_id).encode("utf-8"))
        sha256.update(str(payment.source_payment_id).encode("utf-8"))
        sha256.update(str(payment.is_follow_up).encode("utf-8"))
        sha256.update(str(payment.reason_for_unsuccessful_payment).encode("utf-8"))
        sha256.update(str(payment.program_id).encode("utf-8"))
        sha256.update(str(payment.order_number).encode("utf-8"))
        sha256.update(str(payment.token_number).encode("utf-8"))
        sha256.update(str(payment.household_snapshot.snapshot_data).encode("utf-8"))
        sha256.update(str(payment.business_area_id).encode("utf-8"))
        sha256.update(str(payment.status).encode("utf-8"))
        sha256.update(str(payment.status_date).encode("utf-8"))
        sha256.update(str(payment.household_id).encode("utf-8"))
        sha256.update(str(payment.head_of_household_id).encode("utf-8"))
        sha256.update(str(payment.delivery_type).encode("utf-8"))
        sha256.update(str(payment.currency).encode("utf-8"))
        sha256.update(str(payment.entitlement_quantity).encode("utf-8"))
        sha256.update(str(payment.entitlement_quantity_usd).encode("utf-8"))
        sha256.update(str(payment.delivered_quantity).encode("utf-8"))
        sha256.update(str(payment.delivered_quantity_usd).encode("utf-8"))
        sha256.update(str(payment.delivery_date).encode("utf-8"))
        sha256.update(str(payment.transaction_reference_id).encode("utf-8"))
