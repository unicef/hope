...
class TestBackfillWuAdInvoices(TestCase):
    def test_get_target_data_files_returns_only_2026_non_error_rows_with_amount(self) -> None:
        ...
        self.assertEqual(target_rows, [included_completed])
        self.assertNotIn(excluded_old, target_rows)
        self.assertNotIn(excluded_error, target_rows)
        self.assertNotIn(excluded_missing_amount, target_rows)

    def test_load_invoice_candidates_only_returns_2026_ad_files_not_already_imported(self) -> None:
        ...
        service.ftp_client.INVOICE_PREFIX_PATTERN.match.side_effect = (
            lambda value: value.startswith("AD-")
        )
        ...
        self.assertEqual([candidate.filename for candidate in candidates], ["AD-AUS001029-SL-20260115.ZIP"])

    def test_load_invoice_candidates_respects_invoice_name_and_limit(self) -> None:
        service = Mock()
        service.ftp_client.list_files_w_attrs.return_value = [
            SimpleNamespace(filename="AD-AUS001029-SL-20260115.ZIP"),
        ...
        limited_candidates = list(load_invoice_candidates(service, limit=1))

        self.assertEqual([candidate.filename for candidate in named_candidates], ["AD-AUS001029-SL-20260116.ZIP"])
        self.assertEqual(len(limited_candidates), 1)

    def test_backfill_dry_run_only_previews_matches(self) -> None:
        ...
        with (
            ...
            patch("hope.one_time_scripts.backfill_wu_ad_invoices.import_invoice_candidates") as import_invoice_candidates_mock,
        ):
    ...
    def test_backfill_reprocesses_only_non_legacy_2026_pending_invoices_without_notifications(self) -> None:
        WesternUnionData.objects.create(
            name="QCF-AUS001029-SL-20260115.ZIP",
            date=date(2026, 1, 15),
            amount=Decimal("100.00"),
        ...
        with (
            ...
            patch("hope.one_time_scripts.backfill_wu_ad_invoices.reconcile_backfill_invoice") as reconcile_backfill_invoice_mock,
        ):
            backfill_wu_ad_invoices(dry_run=False)

        self.assertEqual(service.reconcile_invoice.call_count, 1)
        first_invoice = service.reconcile_invoice.call_args_list[0].args[0]
        self.assertEqual(first_invoice.id, waiting_invoice.id)
        self.assertEqual(
            service.get_matching_data_file.call_args_list[0].kwargs,
            {"include_completed": True},
        )
        self.assertEqual(
            service.get_matching_data_file.call_args_list[1].kwargs,
            {"include_completed": True},
        )
        self.assertEqual(
            service.reconcile_invoice.call_args_list[0].kwargs,
            {"send_notifications": False},
        )
        reconcile_backfill_invoice_mock.assert_called_once_with(service, matched_invoice, data_file)
        self.assertEqual(legacy_invoice.status, WesternUnionInvoice.STATUS_COMPLETED)
        self.assertEqual(old_invoice.status, WesternUnionInvoice.STATUS_PENDING)

    def test_reconcile_backfill_invoice_processes_completed_data_without_notifications(self) -> None:
        ...
        self.assertEqual(invoice.status, WesternUnionInvoice.STATUS_COMPLETED)
        self.assertEqual(invoice.matched_data, matched_data)
        self.assertEqual(matched_data.status, WesternUnionData.STATUS_COMPLETED)

    def test_backfill_passes_invoice_name_to_candidate_loading_without_filtering_data_rows_by_ad_name(self) -> None:
        WesternUnionData.objects.create(
            name="QCF-AUS001029-SL-20260115.ZIP",
            date=date(2026, 1, 15),
            amount=Decimal("100.00"),
        ...
        with (
            patch("hope.one_time_scripts.backfill_wu_ad_invoices.QCFReportsService", return_value=service),
            patch("hope.one_time_scripts.backfill_wu_ad_invoices.load_invoice_candidates", return_value=[]) as load_candidates_mock,
    ...
    def test_preview_matching_data_file_uses_non_legacy_invoice_stub(self) -> None:
        service = Mock()
        ...
        self.assertEqual(result, matched_data)
        invoice_stub = service.get_matching_data_file.call_args.args[0]
        self.assertFalse(invoice_stub.is_legacy)
        self.assertEqual(service.get_matching_data_file.call_args.kwargs, {"include_completed": True})
