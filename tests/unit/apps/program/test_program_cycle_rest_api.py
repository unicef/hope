...
class ProgramCycleAPITestCase(HOPEApiTestCase):
    ...
    def test_delete_program_cycle(self) -> None:
        ...
        self.assertIn(
            "Don’t allow to delete Cycle with assigned Target Population",
            bad_response.data,
        )
        tp.delete()
...
class ProgramCycleCreateSerializerTest(TestCase):
    ...
    def test_validate_if_no_end_date(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "All Programme Cycles must have an end date before creating a new one.",
            str(error.exception),
        )
    ...
    def test_validate_program_status(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle can only be created for an Active Programme.",
            str(error.exception),
        )
    ...
    def test_validate_start_date(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle start date must be between programme start and end dates.",
            str(error.exception),
        )
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle start date cannot be before programme start date.",
            str(error.exception),
        )
        # no error
        data = {
    ...
    def test_validate_end_date(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle end date cannot be before programme start date.",
            str(error.exception),
        )
...
class ProgramCycleUpdateSerializerTest(TestCase):
    ...
    def test_validate_program_status(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Updating Programme Cycle is only possible for Active Programme.",
            str(error.exception),
        )
    ...
    def test_validate_start_date(self) -> None:
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle start date must be within the programme's start and end dates",
            str(error.exception),
        )
        # after program end date
        serializer = ProgramCycleUpdateSerializer(
        ...
        with self.assertRaises(ValidationError) as error:
            serializer.is_valid(raise_exception=True)
        self.assertIn(
            "Programme Cycle start date must be after the programme start date.",
            str(error.exception),
        )
        # no error
        serializer = ProgramCycleUpdateSerializer(
...
class ProgramCycleViewSetTestCase(TestCase):
    ...
    def test_delete_non_active_program(self) -> None:
        ...
        self.assertEqual(
            ...
            "Only Programme Cycle for Active Programme can be deleted.",
        )  # type: ignore
