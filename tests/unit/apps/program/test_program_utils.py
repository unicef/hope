...
class TestEnrolHouseholdToProgram(TestCase):
    ...
    def test_enroll_household_to_program(self) -> None:
        ...
        assert hh_count + 1 == Household.original_and_repr_objects.count()
        # 2 new individuals enrolled to program2, individual1 and individual_hoh, individual2 was already in program 2
        assert ind_count + 2 == Individual.original_and_repr_objects.count()
        ...
        assert bank_account_info_count + 1 == BankAccountInfo.objects.count()
        ...
        assert enrolled_household.individuals.count() == original_household.individuals.count(), 2
