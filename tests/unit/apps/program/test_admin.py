...
def _program_add_form_data(
    ...
    **overrides: object,
    ...
    data: dict[str, Any] = {
        ...
        "data_collecting_type": data_collecting_type.pk,
        "beneficiary_group": beneficiary_group.pk,
        "collision_detector": "hope.apps.program.collision_detectors.NoopCollisionDetector",
        "household_count": 0,
        "individual_count": 0,
        "reconciliation_window_in_days": 0,
        "is_visible": True,
        "cycles-TOTAL_FORMS": "0",
        "cycles-INITIAL_FORMS": "0",
        "Program_payment_plan_purposes-TOTAL_FORMS": "0",
        "Program_payment_plan_purposes-INITIAL_FORMS": "0",
    }
    data.update(overrides)
    return data
...
@pytest.fixture
def program_adder(db: Any) -> User:
    program_adder = UserFactory(username="program-adder", is_staff=True, is_superuser=False)
    program_adder.user_permissions.add(
        *Permission.objects.filter(
            codename__in=("add_program", "view_program"),
            content_type=get_content_type_for_model(Program),
        )
    )
    return program_adder


@pytest.fixture
def standard_data_collecting_type(db: Any) -> DataCollectingType:
    return DataCollectingTypeFactory(type=DataCollectingType.Type.STANDARD)


@pytest.fixture
def master_detail_beneficiary_group(db: Any) -> BeneficiaryGroup:
    return BeneficiaryGroupFactory(master_detail=True)


@pytest.fixture
def all_except_cw_business_area(db: Any) -> BusinessArea:
    return BusinessAreaFactory(ingest_source=BusinessArea.IngestSource.ALL_EXCEPT_COUNTRY_WORKSPACE)


def test_add_view_rejects_biometric_flag_on_non_cw_business_area(
    django_app_no_csrf: Any,
    program_adder: User,
    standard_data_collecting_type: DataCollectingType,
    master_detail_beneficiary_group: BeneficiaryGroup,
    all_except_cw_business_area: BusinessArea,
) -> None:
    response = django_app_no_csrf.post(
        reverse("admin:program_program_add"),
        user=program_adder,
        params=_program_add_form_data(
            all_except_cw_business_area,
            standard_data_collecting_type,
            master_detail_beneficiary_group,
    ...
    assert "Biometric deduplication cannot be enabled" in response.text
    assert not Program.all_objects.filter(name="Program added in admin").exists()


def test_add_view_accepts_biometric_flag_on_cw_business_area(
    django_app_no_csrf: Any,
    program_adder: User,
    standard_data_collecting_type: DataCollectingType,
    master_detail_beneficiary_group: BeneficiaryGroup,
    ...
    assert response.status_code == status.HTTP_302_FOUND
    assert Program.objects.get(name="Program added in admin").biometric_deduplication_enabled is True
