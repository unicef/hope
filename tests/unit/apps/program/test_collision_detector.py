...
def test_collision_should_raise_error_when_deleting_individual_with_payment(
    program: Program,
    admin1: Area,
) -> None:
    """Test that collision detector raises error when trying to delete an individual who has a Payment.

    Scenario:
    - Destination household has 2 individuals: IND-KEY-001 and IND-KEY-TO-DELETE
    - Source household has 1 individual: IND-KEY-001
    - Normally IND-KEY-TO-DELETE would be deleted (not in source)
    - BUT IND-KEY-TO-DELETE has a Payment, so merge should FAIL with explicit error

    Expected behavior:
    - ValueError should be raised with information about the individual with payment
    - No data should be modified (atomic transaction rollback)
    """
    ...
    destination_household, destination_individuals = create_household_and_individuals(
        ...
        individuals_data=[
            ...
            },
            {
                ...
                "business_area": program.business_area,
                "sex": MALE,
                "identification_key": "IND-KEY-TO-DELETE",
            },
        ],
    )

    individual_with_payment = destination_individuals[1]
    assert individual_with_payment.identification_key == "IND-KEY-TO-DELETE"

    IndividualRoleInHousehold.objects.create(
        individual=individual_with_payment,
    ...
    Payment.objects.create(
        ...
        program=program,
        household=destination_household,
        head_of_household=destination_individuals[0],
        collector=individual_with_payment,
    ...
    with pytest.raises(ValueError, match=r"Cannot remove individuals with existing payments.*IND-DEST-002"):
        detector.update_household(source_household)

    assert Individual.all_objects.filter(id=individual_with_payment.id).exists()
