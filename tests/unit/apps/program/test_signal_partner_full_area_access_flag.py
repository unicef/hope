...
class TestPartnerFullAreaAccessSignal(TestCase):
    ...
    def test_create_program_full_access_area_flag(self) -> None:
        ...
        assert new_program_partner_through.full_area_access is True
        assert self.program.program_partner_through.filter(partner=new_partner).first().areas.count() == 2
        assert (
            self.area_in_afg_1 in self.program.program_partner_through.filter(partner=new_partner).first().areas.all()
        )
        assert (
            self.area_in_afg_2 in self.program.program_partner_through.filter(partner=new_partner).first().areas.all()
        )
        assert (
            self.area_not_in_afg
            not in self.program.program_partner_through.filter(partner=new_partner).first().areas.all()
        )

    def test_update_program_full_access_area_flag(self) -> None:
        assert self.program.program_partner_through.filter(partner=self.partner).first().full_area_access is False
        assert self.program.program_partner_through.filter(partner=self.partner).first().areas.count() == 1
        assert (
            self.area_in_afg_1 in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
        )
        assert (
            self.area_in_afg_2
            not in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
        )
        assert (
            self.area_not_in_afg
            not in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
        ...
        assert self.program.program_partner_through.filter(partner=self.partner).first().full_area_access is True
        assert self.program.program_partner_through.filter(partner=self.partner).first().areas.count() == 2
        assert (
            self.area_in_afg_1 in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
        )
        assert (
            self.area_in_afg_2 in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
        )
        assert (
            self.area_not_in_afg
            not in self.program.program_partner_through.filter(partner=self.partner).first().areas.all()
