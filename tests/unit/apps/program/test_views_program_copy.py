...
def test_copy_program_with_permission(
    ...
    assert response.json() == {"message": f"Program copied successfully. New Program code: {new_program.code}"}
...
def test_copy_program(
    ...
    new_code = response.json()["message"].split(": ")[-1]
    new_program = Program.objects.get(code=new_code, business_area=afghanistan)

    # Check copied fields
    assert new_program.name == "Copied Program Name"
    assert new_program.code == "copy"
    ...
    assert original_program.code == "orig"
...
def test_copy_program_new_code_generation(
    ...
    new_code = response.json()["message"].split(": ")[-1]
    new_program = Program.objects.get(code=new_code, business_area=afghanistan)
    assert new_program.code != program_to_copy.code
