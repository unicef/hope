...
def test_script_dry_run_reports_grouped_counts_without_updating(
    ...
    assert "Found 4 candidate payments across 2 business area/program groups." in output
    assert "[1/2] ineligible-backfill-first / Ineligible Backfill First: 2 candidates (dry run)." in output
    assert "[2/2] ineligible-backfill-second / Ineligible Backfill Second: 2 candidates (dry run)." in output
    assert "Dry run complete: 4 payments would be marked across 2 business area/program groups." in output
...
def test_script_updates_each_group_atomically_and_preserves_other_records(
    ...
    assert "[1/2] ineligible-backfill-first / Ineligible Backfill First: updating 2 candidates..." in output
    assert "[1/2] ineligible-backfill-first / Ineligible Backfill First: committed 2 updates." in output
    assert "Live run complete: committed 4 updates across 2 business area/program groups." in output
