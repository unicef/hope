...
class TestApplyMappedValue:
    """Tests for GenericRegistrationService._apply_mapped_value classmethod."""

    def test_individual_field_sets_value_on_dict(self):
        my_dict = {}
        GenericRegistrationService._apply_mapped_value(my_dict, INDIVIDUAL_FIELD, "given_name", "John")
        assert my_dict["given_name"] == "John"

    def test_individual_field_overwrites_existing_value(self):
    ...
    def test_document_field_creates_new_doc_entry(self):
        my_dict = {"documents": {}}
    ...
    def test_document_field_updates_existing_doc_entry(self):
        my_dict = {"documents": {"doc_tax": {"key": "tax_id"}}}
        GenericRegistrationService._apply_mapped_value(my_dict, DOCUMENT_FIELD, "doc_tax-document_number", "ABC123")
        assert my_dict["documents"]["doc_tax"]["key"] == "tax_id"
        assert my_dict["documents"]["doc_tax"]["document_number"] == "ABC123"

    def test_document_field_parses_field_with_dash(self):
        my_dict = {"documents": {}}
    ...
    def test_document_field_multiple_documents(self):
        my_dict = {"documents": {}}
    ...
    def test_account_field_sets_value(self):
    ...
    def test_account_field_multiple_fields(self):
        my_dict = {ACCOUNT_FIELD: {}}
        GenericRegistrationService._apply_mapped_value(my_dict, ACCOUNT_FIELD, "bank_name", "UNICEF Bank")
    ...
    def test_extra_field_sets_value(self):
        my_dict = {"extra": {}}
    ...
    def test_extra_field_multiple_fields(self):
        my_dict = {"extra": {}}
    ...
    def test_unknown_model_does_not_modify_dict(self):
...
class TestAssignIndividualRoles:
    """Tests for GenericRegistrationService._assign_individual_roles instance method."""
    ...
    @staticmethod
    def _make_individual(relationship=""):
        individual = MagicMock()
        individual.relationship = relationship
        return individual

    def test_head_assignment(self, service):
        individual = self._make_individual(relationship=HEAD)
    ...
    def test_duplicate_head_raises_validation_error(self, service):
        individual = self._make_individual(relationship=HEAD)
        existing_head = self._make_individual(relationship=HEAD)
    ...
    def test_primary_collector_assignment(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
    ...
    def test_primary_collector_with_yes_string(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
        extra_data = {PRIMARY_COLLECTOR: "y"}
        head, pr_collector, sec_collector = service._assign_individual_roles(individual, extra_data, None, None, None)
        assert pr_collector is individual

    def test_duplicate_primary_collector_raises_validation_error(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
        existing_pr_collector = self._make_individual()
        extra_data = {PRIMARY_COLLECTOR: "1"}
    ...
    def test_secondary_collector_assignment(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
    ...
    def test_duplicate_secondary_collector_raises_validation_error(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
        existing_sec_collector = self._make_individual()
        extra_data = {SECONDARY_COLLECTOR: "1"}
    ...
    def test_head_and_primary_collector_combined(self, service):
        individual = self._make_individual(relationship=HEAD)
        extra_data = {PRIMARY_COLLECTOR: "y"}
        head, pr_collector, sec_collector = service._assign_individual_roles(individual, extra_data, None, None, None)
        assert head is individual
    ...
    def test_non_head_without_collector_flags(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
        head, pr_collector, sec_collector = service._assign_individual_roles(individual, {}, None, None, None)
        assert head is None
        assert pr_collector is None
    ...
    def test_preserves_existing_head_when_not_head(self, service):
        existing_head = self._make_individual(relationship=HEAD)
        individual = self._make_individual(relationship="SON_DAUGHTER")
    ...
    def test_preserves_existing_collectors_when_no_flags(self, service):
        existing_pr = self._make_individual()
        existing_sec = self._make_individual()
        individual = self._make_individual(relationship="SON_DAUGHTER")
    ...
    def test_primary_collector_false_does_not_assign(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
        extra_data = {PRIMARY_COLLECTOR: "0"}
        head, pr_collector, sec_collector = service._assign_individual_roles(individual, extra_data, None, None, None)
        assert pr_collector is None

    def test_secondary_collector_false_does_not_assign(self, service):
        individual = self._make_individual(relationship="SON_DAUGHTER")
