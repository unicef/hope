...
@frozenfixture()
def import_data(request, db, business_area):
    return ImportDataFactory(business_area_slug=business_area.slug)


def test_retrieve_import_data(superuser, business_area, program, role_assignment, import_data):
    recorder = HopeRecorder(DATA_DIR, as_user=superuser)
    recorder.assertGET(f"/api/rest/business-areas/{business_area.slug}/import-data/{import_data.pk}/")
