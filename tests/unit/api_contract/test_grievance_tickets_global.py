...
@frozenfixture()
def grievance_ticket(request, db, business_area, superuser):
    return GrievanceTicketFactory(business_area=business_area, created_by=superuser, assigned_to=superuser)
...
def test_retrieve_grievance_ticket(superuser, business_area, program, role_assignment, grievance_ticket):
    recorder = HopeRecorder(DATA_DIR, as_user=superuser)
    recorder.assertGET(f"/api/rest/business-areas/{business_area.slug}/grievance-tickets/{grievance_ticket.pk}/")
