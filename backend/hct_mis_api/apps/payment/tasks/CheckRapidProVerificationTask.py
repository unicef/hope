...
class CheckRapidProVerificationTask:
    ...
    def _verify_cashplan_payment_verification(self, cashplan_payment_verification):
        payment_record_verifications = (
            cashplan_payment_verification.payment_record_verifications.prefetch_related(
        ...
        payment_record_verifications_phone_number_dict = {
            str(x.payment_record.head_of_household.phone_no): x
            for x in payment_record_verifications
        }
        api = RapidProAPI(business_area.slug)
        rapid_pro_results = api.get_mapped_flow_runs(
            cashplan_payment_verification.rapid_pro_flow_start_uuids
        )
        payment_record_verification_to_update = (
            self._get_payment_record_verification_to_update(
    ...
    def _get_payment_record_verification_to_update(self, results, phone_numbers):
        ...
        for rapid_pro_result in results:
            payment_record_verification = (
                self._rapid_pro_results_to_payment_record_verification(
    ...
    def _rapid_pro_results_to_payment_record_verification(
        ...
        if not phone_number or not is_right_phone_number_format(phone_number):
            return None
        payment_record_verification = (
            payment_record_verifications_phone_number_dict.get(phone_number)
        )
