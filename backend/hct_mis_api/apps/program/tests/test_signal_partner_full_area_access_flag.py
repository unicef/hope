...
class TestPartnerFullAreaAccessSignal(TestCase):
    ...
    def test_create_program_full_access_area_flag(self) -> None:
        ...
        self.assertIn(self.area_in_afg_1, self.program.program_partner_through.filter(partner=new_partner).first().areas.all())
        self.assertIn(self.area_in_afg_2, self.program.program_partner_through.filter(partner=new_partner).first().areas.all())
        self.assertNotIn(self.area_not_in_afg, self.program.program_partner_through.filter(partner=new_partner).first().areas.all())

    def test_update_program_full_access_area_flag(self) -> None:
        self.assertEqual(self.program.program_partner_through.filter(partner=self.partner).first().full_area_access, False)
        self.assertEqual(self.program.program_partner_through.filter(partner=self.partner).first().areas.count(), 1)
        self.assertIn(self.area_in_afg_1, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
        self.assertNotIn(self.area_in_afg_2, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
        self.assertNotIn(self.area_not_in_afg, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
        ...
        self.assertEqual(self.program.program_partner_through.filter(partner=self.partner).first().full_area_access, True)
        self.assertEqual(self.program.program_partner_through.filter(partner=self.partner).first().areas.count(), 2)
        self.assertIn(self.area_in_afg_1, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
        self.assertIn(self.area_in_afg_2, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
        self.assertNotIn(self.area_not_in_afg, self.program.program_partner_through.filter(partner=self.partner).first().areas.all())
