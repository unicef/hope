...
from hct_mis_api.apps.core.models import DataCollectingType, BusinessAreaPartnerThrough
...
def partner_permission_removed(sender: Any, instance: BusinessAreaPartnerThrough, **kwargs: Any) -> None:
    """
    If permissions are revoked for a Partner from a whole Business Area, Partner loses access to all Programs in this Business Area
    """
    ...
    ProgramPartnerThrough.objects.filter(
        partner=partner,
        program__in=programs_in_business_area
    ).delete()
