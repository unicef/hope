...
def core_fields_to_separated_dict(core_fields_attrs):
    result_dict = {
        "individuals": {},
        "households": {},
    }
    ...
    LazyEvalMethodsDict(
        {
            ...
            "hint": "",
            "choices": lambda *args, **kwargs: Area.get_admin_areas_as_choices(),
            "associated_with": _HOUSEHOLD,
...
class FieldFactory(list):
    all_fields = CORE_FIELDS_ATTRIBUTES
    ...
    def from_scopes(cls, scopes: list[Scope]):
        def has_any(field):
            return any(True for scope in scopes if scope in field["scope"])

        factory = cls()
        factory.extend(filter(has_any, cls.all_fields))
    ...
    def from_scope(cls, scope: Scope):
        factory = cls()
        factory.extend(filter(lambda field: scope in field["scope"], cls.all_fields))
    ...
    def and_scope(self, scope: Scope):
        ...
        if scope not in self.scopes:
            factory.extend(filter(lambda field: scope in field["scope"], self.all_fields))
            factory.scopes.add(scope)
        return factory
    ...
    def apply_business_area(self, business_area_slug: str):
        ...
        for field in factory:
            ...
            if callable(choices):
                field["choices"] = choices(business_area_slug)
        return factory
