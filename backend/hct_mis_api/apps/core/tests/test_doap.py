...
class TestDOAP(WebTest):
    ...
    def test_show_doap(self):
        url = reverse(
            "admin:core_businessarea_view_ca_doap", args=[self.business_area.pk]
        )
    ...
    def test_send_doap(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, "CashAssist - UNICEF - Afghanistan user updates"
        )

    def test_export_doap(self):
        url = reverse(
            "admin:core_businessarea_export_doap", args=[self.business_area.pk]
        )
        res = self.app.get(url, user=self.user)
