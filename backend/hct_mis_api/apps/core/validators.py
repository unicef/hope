...
def raise_program_status_is(status: str) -> typing.Callable:
    """This wrapper checks if actions can be proceeded for program not in given status"""

    def decorator(func: typing.Callable) -> typing.Callable:
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            if len(args) >= 3 and isinstance(args[2], ResolveInfo):  # gets info object
                info = args[2]
            else:
                raise Exception("Resolve object is missing")

            program_id = decode_id_string_required(info.context.headers.get("Program"))
            program = Program.objects.get(id=program_id)

            if program.status == status:
                raise ValidationError(f"In order to proceed this action, program status cannot be {status}")
