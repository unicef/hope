...
class GoldenRecordTargetingCriteriaAgeFilterQueryTestCase(APITestCase):
    ...
    def setUpTestData(cls):
        ...
        individuals1[0].birth_date = datetime.date.today() - relativedelta(
            years=+20, days=+5
        )  # age 20
        individuals1[1].birth_date = datetime.date.today() - relativedelta(
            years=+22, days=-5
        )  # age 21
        individuals2[0].birth_date = datetime.date.today() - relativedelta(
            years=+24, days=+5
        )  # age 24
        individuals2[1].birth_date = datetime.date.today() - relativedelta(
            years=+26, days=-5
        )  # age 25
    ...
    def test_filter_records_by_age(self, arguments, method):
        self.create_user_role_with_permissions(
            self.user, [Permissions.TARGETING_VIEW_DETAILS], self.business_area
        )
        variables = {
