...
class TestDetails(TestCase):
    ...
    def test_filtering_business_area_code_with_tax_id(self):
        ...
        response_nok = self.api_client.get(
            f"/api/hh-status?tax_id={tax_id}&business_area_code=non-existent"
        )
    ...
    def test_filtering_business_area_code_with_registration_id(self):
        ...
        imported_individual = ImportedIndividualFactory(
            household=imported_household, relationship=HEAD
        )
    ...
    def test_getting_individual_with_status_imported(self):
        ...
        imported_document = ImportedDocumentFactory(
    ...
    def test_getting_individual_with_status_targeted(self):
        ...
        self.assertEqual(
            info["date"], _time(HouseholdSelection.objects.first().updated_at)
        )
    ...
    def test_getting_household_with_status_imported(self):
        ...
        response = self.api_client.get(
            f"/api/hh-status?registration_id={registration_id}"
        )
        self.assertEqual(response.status_code, 200)
        data = response.json()
        info = data["info"]
