from django.db.models import Prefetch, Sum, Value, Count, Q
...
class Query(graphene.ObjectType):
    ...
    def resolve_all_households(self, info, **kwargs):
        ...
        if does_path_exist_in_query("edges.node.headOfHousehold", info):
            queryset = queryset.prefetch_related("head_of_household")
        if does_path_exist_in_query("edges.node.hasDuplicates", info):
            queryset = queryset.annotate(
                has_duplicates_annotated=Count("individuals", filter=Q(individuals__deduplication_golden_record_status=DUPLICATE))
            )

        if does_path_exist_in_query("edges.node.sanctionListPossibleMatch", info):
            queryset = queryset.annotate(
                sanction_list_possible_match_annotated=Count(
                    "individuals", filter=Q(individuals__sanction_list_possible_match=True)
                )
            )
        if does_path_exist_in_query("edges.node.sanctionListConfirmedMatch", info):
            queryset = queryset.annotate(
                sanction_list_confirmed_match_annotated=Count(
                    "individuals", filter=Q(individuals__sanction_list_confirmed_match=True)
                )
            )
        return queryset
