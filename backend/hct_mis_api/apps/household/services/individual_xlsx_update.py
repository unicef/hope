...
class IndividualXlsxUpdate:
    ...
    def __init__(self, xlsx_update_file):
        our_attributes = FieldFactory.from_scopes(
            [Scope.GLOBAL, Scope.INDIVIDUAL_XLSX_UPDATE]
        )
        self.xlsx_update_file = xlsx_update_file
        self.core_attr_by_names = {
            self._column_name_by_attr(attr): attr for attr in our_attributes
        }
        self.updatable_core_columns_names = [
            self._column_name_by_attr(attr)
            for attr in FieldFactory.from_scope(
                Scope.GLOBAL
    ...
    def get_matching_report(self):
        ...
        for row in self.individuals_ws.iter_rows(
            min_row=IndividualXlsxUpdate.DATA_ROW_INDEX
        ):
    ...
    def update_individuals(self):
        ...
        for individuals_unique_report in self.report_dict[
            IndividualXlsxUpdate.STATUS_UNIQUE
        ]:
    ...
    def _validate_columns_names(self):
        ...
        invalid_columns = [
            ...
            if cell.value not in self.core_attr_by_names
        ]
    ...
    def _build_helpers(self):
        ...
        self.attr_by_column_index = {
            cell.col_idx: self.core_attr_by_names[cell.value] for cell in first_row
        }
        self.columns_match_indexes = [
            self.columns_names_index_dict[col] for col in self.xlsx_match_columns
        ]
        return self.columns_names

    def _get_queryset(self):
        queryset = Individual.objects.filter(
            business_area=self.xlsx_update_file.business_area
        )

        if self.xlsx_update_file.rdi:
            queryset = queryset.filter(
                registration_data_import=self.xlsx_update_file.rdi
            )
