...
class PullFromErpDatahubTask:
    ...
    def update_cash_plans(self):
        cash_plans_without_exchange_rate = CashPlan.objects.filter(
        ...
        CashPlan.objects.bulk_update(
            cash_plans_without_exchange_rate, ["exchange_rate"]
        )
    ...
    def update_payment_records(self):
        ...
        for payment_record in payment_records_to_update:
            payment_record.delivered_quantity_usd = (
                utils.get_payment_record_delivered_quantity_in_usd(payment_record)
            )

        PaymentRecord.objects.bulk_update(
            payment_records_to_update, ["delivered_quantity_usd"]
        )
