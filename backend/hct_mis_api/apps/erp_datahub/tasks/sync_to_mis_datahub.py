...
class SyncToMisDatahubTask:
    ...
    def execute(self):
        ...
        parent_business_area_codes = list(
            BusinessArea.objects.filter(is_split=True).values_list("code", flat=True)
        )
        normal_down_payments_to_sent = DownPayment.objects.exclude(
        ...
        for down_payment in normal_down_payments_to_sent:
            mis_down_payment = mis_models.DownPayment(
                **SyncToMisDatahubTask.get_model_dict(down_payment)
            )
        ...
        for funds_commitment in normal_funds_commitments_to_sent:
            mis_funds_commitment = mis_models.FundsCommitment(
                **SyncToMisDatahubTask.get_model_dict(funds_commitment)
            )
        ...
        normal_down_payments_to_sent.update(
            mis_sync_flag=True, mis_sync_date=timezone.now()
        )
        normal_funds_commitments_to_sent.update(
            mis_sync_flag=True, mis_sync_date=timezone.now()
        )
        down_payments_from_split_business_areas.update(
            mis_sync_flag=True, mis_sync_date=timezone.now()
        )
        funds_commitments_from_split_business_areas.update(
            mis_sync_flag=True, mis_sync_date=timezone.now()
        )
