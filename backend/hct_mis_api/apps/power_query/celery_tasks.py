...
class ReportQueryTask(AbstractPowerQueryTask):
    model = Report
...
@app.task(bind=True, default_retry_delay=60, max_retries=3, base=ReportQueryTask)
@sentry_tags
...
@app.task(bind=True, default_retry_delay=60, max_retries=3, base=PowerQueryTask)
@sentry_tags
    ...
    try:
        for report in Report.objects.filter(active=True, frequence__isnull=False):
            if should_run(report.frequence):
                ret: Any = report.execute(run_query=True)
                results.append(ret)
            else:
