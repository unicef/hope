...
from .forms import QueryForm, ExportForm
from .models import Query, Dataset, Formatter, Report
...
class QueryAdmin(ImportExportMixin, ExtraUrlMixin, ModelAdmin):
    ...
    search_fields = ("name",)
    list_filter = (('target', AutoCompleteFilter),
    ...
    def preview(self, request, pk):
        ...
        try:
            ...
            if isinstance(ret, QuerySet):
                ret = ret[:100]
            context['results'] = ret
            return render(request, "power_query/results.html", context)
        except Exception as e:
            self.message_user(request, f'{e.__class__.__name__}: {e}', messages.ERROR)

    def get_changeform_initial_data(self, request):
        ct = ContentType.objects.filter(id=request.GET.get('ct', 0)).first()
        return {'code': "result=conn.all()",
                'name': ct,
                'target': ct,
                "owner": request.user}
