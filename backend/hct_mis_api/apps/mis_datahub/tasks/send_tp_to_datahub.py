...
from hct_mis_api.apps.core.utils import build_arg_dict, nested_getattr
...
class SendTPToDatahubTask:
            ...
            with transaction.atomic(using="default"):
                with transaction.atomic(using="cash_assist_datahub_mis"):
                    ...
                    for individual in individuals_to_sync:
                        dh_individual = self._prepare_datahub_object_individual(
                            individual
                        )
                    ...
                    for selection in target_population_selections:
                        dh_target_population_selection = (
                            self._prepare_datahub_object_target_entry(selection)
                        )
                        tp_entries_to_bulk_create.append(dh_target_population_selection)

                    dh_mis_models.Household.objects.bulk_create(
                        households_to_bulk_create
                    )
                    dh_mis_models.Individual.objects.bulk_create(
                        individuals_to_bulk_create
                    )
                    dh_mis_models.IndividualRoleInHousehold.objects.bulk_create(
                        roles_to_bulk_create
                    )
    ...
    def _prepare_data_to_send(self, program, target_population):
        ...
        roles_to_sync = IndividualRoleInHousehold.objects.filter(
            Q(household__last_sync_at__isnull=True)
            | Q(household__last_sync_at__lte=F("household__updated_at")),
            household_id__in=all_targeted_households_ids,
        ).distinct()
        return (
            documents,
    ...
    def _prepare_unhcr_dict(self, individuals_to_sync):
        ...
        self.unhcr_id_dict = {
            identity.individual_id: identity.number
            for identity in individual_identities
        }

    def _get_individuals_and_hauseholds(self, program, target_population):
        all_targeted_households_ids = target_population.final_list.values_list(
            "id", flat=True
        )
    ...
    def _send_program(self, program):
        if not (
            program.last_sync_at is None or program.last_sync_at < program.updated_at
        ):
            return
        dh_program_args = build_arg_dict(
    ...
    def _prepare_datahub_object_household(self, household):
        dh_household_args = build_arg_dict(
            household, SendTPToDatahubTask.MAPPING_HOUSEHOLD_DICT
        )
        dh_household_args["country"] = CountryCodeMap.objects.get_code(
            household.country.code
        )
    ...
    def _prepare_datahub_object_individual(self, individual):
        dh_individual_args = build_arg_dict(
    ...
    def _prepare_datahub_object_document(self, document):
        dh_document_args = build_arg_dict(
    ...
    def _prepare_datahub_object_target_entry(self, target_population_selection):
        household_unhcr_id = self._get_unhcr_household_id(
