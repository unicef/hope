...
class RegistrationXlsxImportMutation(BaseValidator, PermissionMutation, ValidationErrorMutationMixin):
    ...
    def processed_mutate(
        ...
        log_create(
            RegistrationDataImport.ACTIVITY_LOG_MAPPING, "business_area", info.context.user, created_obj_hct.program_id, None, created_obj_hct
        )
...
class RegistrationDeduplicationMutation(BaseValidator, PermissionMutation):
    ...
    def mutate(
        ...
        rdi_obj.save()
        log_create(
            RegistrationDataImport.ACTIVITY_LOG_MAPPING, "business_area", info.context.user, rdi_obj.program_id, old_rdi_obj, rdi_obj
        )
...
class MergeRegistrationDataImportMutation(BaseValidator, PermissionMutation):
    ...
    def mutate(cls, root: Any, info: Any, id: Optional[str], **kwargs: Any) -> "MergeRegistrationDataImportMutation":
        ...
        log_create(
            RegistrationDataImport.ACTIVITY_LOG_MAPPING, "business_area", info.context.user, obj_hct.program_id, old_obj_hct, obj_hct
        )
...
class DeleteRegistrationDataImport(graphene.Mutation):
    ...
    def mutate(cls, root: Any, info: Any, **kwargs: Any) -> "DeleteRegistrationDataImport":
        ...
        rdi_obj.delete()
        log_create(RegistrationDataImport.ACTIVITY_LOG_MAPPING, "business_area", info.context.user, program_id, rdi_obj, None)
        return cls(ok=True)
