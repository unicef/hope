...
class TestCzechRepublicRegistrationService(TestCase):
    ...
    def test_import_data_to_datahub(self) -> None:
        ...
        primary_collector = Individual.objects.get(full_name="Tetiana Symkanych")
        ...
        second_child = Individual.objects.get(given_name="TEST")
        ...
        bank_account_info = BankAccountInfo.objects.first()
        ...
        birth_certificate = Document.objects.filter(type__key="birth_certificate").first()
        self.assertEqual(birth_certificate.document_number, "262873")
        self.assertEqual(Document.objects.filter(type__key="disability_card").count(), 1)
        self.assertEqual(Document.objects.filter(type__key="medical_certificate").count(), 1)
        self.assertEqual(Document.objects.filter(type__key="temporary_protection_visa").count(), 1)
        self.assertEqual(Document.objects.filter(type__key="proof_legal_guardianship").count(), 1)
        self.assertEqual(Document.objects.filter(type__key="national_passport").count(), 2)
        self.assertEqual(birth_certificate.rdi_merge_status, "PENDING")

        national_passport = Document.objects.filter(document_number="GB500567").first()
        ...
        disability_card = Document.objects.filter(type__key="disability_card").first()
        ...
        medical_certificate = Document.objects.filter(type__key="medical_certificate").first()
        ...
        temporary_protection_visa = Document.objects.filter(type__key="temporary_protection_visa").first()
        ...
        proof_legal_guardianship = Document.objects.filter(type__key="proof_legal_guardianship").first()
