...
class ImportDataInstanceValidator:
    ...
    def get_combined_attributes(self):
        fields = FieldFactory.from_scopes(
            [Scope.GLOBAL, Scope.XLSX, Scope.HOUSEHOLD_ID]
...
class UploadXLSXInstanceValidator(ImportDataInstanceValidator):
    ...
    def get_combined_fields(self):
        core_fields = FieldFactory.from_scopes(
            [Scope.GLOBAL, Scope.XLSX, Scope.HOUSEHOLD_ID]
        )
...
class KoboProjectImportDataInstanceValidator(ImportDataInstanceValidator):
    ...
    def get_expected_household_fields(self):
        try:
            return {
                field["xlsx_field"]
                for field in self.combined_fields["households"].values()
                if field["required"]
            }
        except Exception as e:
            logger.exception(e)
    ...
    def get_expected_individuals_fields(self):
        try:
            return {
                field["xlsx_field"]
                for field in self.combined_fields["individuals"].values()
                if field["required"]
            }
        except Exception as e:
            logger.exception(e)
    ...
    def validate_everything(self, submissions: list, business_area: BusinessArea):
        try:
            ...
            all_saved_submissions = KoboImportedSubmission.objects.filter(
            ...
            all_saved_submissions = all_saved_submissions.values(
                "kobo_submission_uuid", "kobo_submission_time"
            )

            all_saved_submissions_dict = {}
            for submission in all_saved_submissions:
                item = all_saved_submissions_dict.get(
                    str(submission["kobo_submission_uuid"]), []
                )
            ...
            for household in reduced_submissions:
                submission_exists = household.get(
                    "_submission_time"
                ) in all_saved_submissions_dict.get(household.get("_uuid"), [])
