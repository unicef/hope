...
class RdiDiiaCreateTask(RdiBaseCreateTask):
    ...
    def execute(
        self, registration_data_import_id, diia_hh_ids=None, diia_hh_count=None
    ):
        ...
        registration_data_import_data_hub = (
        ...
        if not diia_household_import_ids:
            ...
            raise ValidationError(
                "Rdi doesn't found any records within status to import"
            )

        registration_data_import_data_hub.import_done = (
        ...
        for diia_household_id in diia_household_import_ids:
            diia_household = DiiaHousehold.objects.defer("source_data").get(
                id=diia_household_id
            )

            try:
                all_individuals = DiiaIndividual.objects.filter(
                    rec_id=diia_household.rec_id
                )
                ...
                for individual in all_individuals:
                    b_date = (
                        dateutil.parser.parse(individual.birth_date, dayfirst=True)
                    ...
                    individual_obj = ImportedIndividual(
                        ...
                        relationship=self.DIIA_RELATION.get(
                            individual.relationship, RELATIONSHIP_UNKNOWN
                        ),
                        ...
                        disability=self.DIIA_DISABILITY_MAP.get(
                            individual.disability, NOT_DISABLED
                        ),
                        registration_data_import=registration_data_import_data_hub,
                        first_registration_date=registration_data_import_data_hub.created_at,
                        last_registration_date=registration_data_import_data_hub.created_at,
                    ...
                    if individual.relationship == HEAD:
                        ...
                        hh_doc = {
                            ...
                            "doc_date": dateutil.parser.parse(
                                individual.doc_issue_date, dayfirst=True
                            )
                    ...
                    if individual.iban:
                        self._add_bank_account(
                            bank_accounts, individual, individual_obj
                        )
                ...
                DiiaIndividual.objects.bulk_update(
                    individuals_to_update_list, ["imported_individual"], 1000
                )
                ...
                diia_household.registration_data_import = (
                    registration_data_import_data_hub
                )
                ...
                logger.exception(
                    f"Error importing DiiaHousehold {diia_household.pk}. {e}"
                )
        ...
        DiiaHousehold.objects.bulk_update(
            households_to_update,
            ["imported_household", "status", "registration_data_import"],
            1000,
        )
        ...
        registration_data_import_data_hub.import_data.number_of_individuals = (
            individual_count
        )
        registration_data_import_data_hub.import_data.number_of_households = len(
            households_to_create
        )
        registration_data_import_data_hub.import_data.status = (
            ImportData.STATUS_FINISHED
        )
    ...
    def _add_hh_doc(self, data, documents):
        ...
        doc_type = (
            self.national_passport_document_type
            if data.get("type") == "passport"
            else self.other_document_type
        )
