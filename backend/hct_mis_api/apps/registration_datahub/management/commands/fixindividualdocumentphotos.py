...
def fix_document_photos():
    imported_documents = (
        ImportedDocument.objects.filter(
            photo="",
            individual__registration_data_import__import_data__data_type=ImportData.JSON,
        )
    ...
    for imported_document in imported_documents:
        ...
        if not rdi:
            print(
                f"Not found RegistrationDataImport object for Imported Document: {imported_document.id}"
            )
            continue
        ...
        try:
            json_file = ImportData.objects.get(
                id=imported_document.import_data_id
            ).file.read()
        except Exception as e:
            print(
                f"Error get or read json file for Imported Document: {imported_document.id}. {e}"
            )
            continue
                    ...
                    for individual in hh_value:
                        ...
                        for i_field, i_value in individual.items():
                            if i_field in RdiKoboCreateTask.DOCS_AND_IDENTITIES_FIELDS:
                                if i_field.endswith("_type_i_c") or i_field.endswith(
                                    "_issuer_i_c"
                                ):
                                    continue
                        ...
                        if (
                            individual_docs_and_identities.get("number")
                            == document_number
                        ):
