...
from hct_mis_api.apps.account.fixtures import PartnerFactory, BusinessAreaFactory
...
class PartnerAccessChangeSignal(TestCase):
    ...
    def setUpTestData(cls) -> None:
        ...
        cls.program_unicef_through = ProgramPartnerThrough.objects.create(program=cls.program, partner=cls.unicef_partner)
        ...
        cls.program_other = ProgramFactory.create(status=Program.DRAFT, business_area=other_ba, partner_access=Program.NONE_PARTNERS_ACCESS)
        cls.program_other_unicef_through = ProgramPartnerThrough.objects.get(program=cls.program_other, partner=cls.unicef_partner)
