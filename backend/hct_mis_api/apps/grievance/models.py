...
class GrievanceTicket(TimeStampedUUIDModel, ConcurrencyModel, UnicefIdentifiedModel):
    ...
    def priority_to_string(self):
        return ES_MAPPING_PRIORITY_CHOICES.get(self.priority)

    def urgency_to_string(self):
        return ES_MAPPING_URGENCY_CHOICES.get(self.urgency)

    def status_to_string(self):
        return ES_MAPPING_STATUS_CHOICES.get(self.status)

    def category_to_string(self):
        return ES_MAPPING_CATEGORY_CHOICES.get(self.category)

    def issue_type_to_string(self):
        if self.category in range(2, 4):
            return ES_MAPPING_ISSUE_TYPES_CHOICES[self.category].get(self.issue_type)
