...
def set_household_unicef_id(apps, schema_editor):
    ...
    start = 10_000
    grievance_tickets = []
    i, count = 0, GrievanceTicket.objects.count() // start + 1
    while i <= count:
        batch = GrievanceTicket.objects.only("issue_type").order_by("created_at")[start * i : start * (i + 1)]
        for grv in batch:
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(set_household_unicef_id, delete_household_unicef_id)]
