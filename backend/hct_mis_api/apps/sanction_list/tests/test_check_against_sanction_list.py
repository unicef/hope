...
class TestCheckAgainstSanctionList(TestCase):
    ...
    def test_check_against_sanction_list(self):
        content = Path(f"{self.TEST_FILES_PATH}/TestSanctionList.xlsx").read_bytes()
        file = File(BytesIO(content), name="TestSanctionList.xlsx")
        uploaded_file_obj = UploadedXLSXFile.objects.create(file=file, associated_email="wnosal@outlook.com")
