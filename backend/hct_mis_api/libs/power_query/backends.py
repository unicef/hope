...
from power_query.models import Report, ReportDocument
...
class PowerQueryBackend(ModelBackend):
    def get_office_permissions(self, user_obj: User, office_slug: str) -> Any:  # type: ignore # Variable "hct_mis_api.libs.power_query.backends.User" is not valid as a type  [valid-type]
        key = f"_perm_{office_slug}"
        if not hasattr(user_obj, key):
            permsissions = Permission.objects.filter(
                group__user_groups__user=user_obj,
                group__user_groups__business_area__slug=office_slug,
            )
            setattr(
                user_obj,
                key,
