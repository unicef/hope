...
def add_accountability_communication_message() -> Message:
    ...
    return CommunicationMessageFactory(
        ...
        title=f"You got credit of USD 100",
        body=f"Greetings, we have sent you USD 100 in your registered account on 2022-09-19 20:00:00 UTC",
        business_area=BusinessArea.objects.first(),
...
class TestSmokeAccountabilityCommunication:
    ...
    def test_smoke_accountability_communication_details(
        ...
        # assert add_accountability_communication_message.created_by in pageAccountabilityCommunicationDetails.getLabelCreatedBy().text
        assert add_accountability_communication_message.data_created in pageAccountabilityCommunicationDetails.getLabelDateCreated().text
        assert "Writer development radio country price." in pageAccountabilityCommunicationDetails.getLabelTargetPopulation().text
        ...
        assert "No results Try adjusting your search or your filters to find what you are looking for ." in pageAccountabilityCommunicationDetails.getTableRow().text.replace("\n", " ")
