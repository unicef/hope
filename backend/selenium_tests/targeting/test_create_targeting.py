...
from hct_mis_api.apps.household.models import Household, HEARING, SEEING, REFUGEE, HOST
...
def sw_program() -> Program:
    return get_program_with_dct_type_and_name("SW Program", dct_type=DataCollectingType.Type.SOCIAL,
                                              status=Program.ACTIVE)


@pytest.fixture
def non_sw_program() -> Program:
    return get_program_with_dct_type_and_name("Non SW Program", dct_type=DataCollectingType.Type.STANDARD,
                                              status=Program.ACTIVE)


@pytest.fixture
def household_with_disability():
...
@pytest.fixture
def household_without_disabilities():
    program = Program.objects.first()
    with transaction.atomic():
        household, individuals = create_household_and_individuals(
            household_data={
...
class TestCreateTargeting:
    def test_create_targeting_for_people(
        ...
        assert pageTargetingDetails.getHouseholdTableCell(1,
                                                          1).text == household_with_disability.individuals.first().unicef_id
    ...
    def test_create_targeting_for_normal_program(
        ...
        assert pageTargetingDetails.getHouseholdTableCell(1,
                                                          1).text == household_refugee.unicef_id
        actions = ActionChains(pageTargetingDetails.driver)
        actions.move_to_element(pageTargetingDetails.getHouseholdTableCell(1,
                                                                           1)).perform()
